...
function buildPrintHTML() {
  ...
  rows.forEach(row => {
    ...
    const rider = row.querySelector('.person-name')?.textContent.trim() || '';
...
function printCSS() {
  ...
  ***************************************************************************/
  ...
  const div = stop.querySelector('div[data-title]');
  if (!div) return '<td></td>';
  ...
  // Obtener solo la dirección (sin repetir el nombre)
  let rawText = div.innerText.trim();
  let address = rawText.replace(name, '').trim();

  // Codificar para URL
  const encoded = encodeURIComponent(address);

  // Enlace: geo en móvil, Google Maps en escritorio
  const link = isMobile
        ...
        <div data-transformed="mobile" style="width:100%; margin-bottom:10px;">
          <table style="width:100%; border-collapse:collapse; border:1px solid #ccc;
            ...
            <thead style="background:#f5f5f5">
              <tr>
                <th style="border:1px solid #ccc; padding:6px; width:20%">Date</th>
